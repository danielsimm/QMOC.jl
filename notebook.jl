...
# ╔═╡ 15dcf6e0-9dee-4f9a-88d3-369bc6652c7d
using DelimitedFiles

# ╔═╡ c563260d-e383-45a7-8a93-6f2dd1c9a251
using JLD2

# ╔═╡ 359446c5-e665-4e87-bca2-6842c5bf0420
using Statistics

# ╔═╡ aef72ffb-42a8-4cc7-9b5f-30fccd7d1f98
using ScalingCollapse

# ╔═╡ 462859f4-1227-11ef-17c4-dbc8028d5c3c
import CairoMakie as cm

# ╔═╡ 4f6cbeb0-1c05-453e-8c01-c510de8fb512
import Meshes as ms

# ╔═╡ dd0dc95e-8e11-48a1-af38-24f7bf8842cb
import GeometryBasics as gb

# ╔═╡ b865d61b-ce7f-4cd4-8e78-5dc2ac3be1fe
begin
	metadata24 = jldopen("cluster/Kitaev24/metadata.jld2")["metadata"]
	metadata28 = jldopen("cluster/Kitaev28/metadata.jld2")["metadata"]
	metadata32 = jldopen("cluster/Kitaev32/metadata.jld2")["metadata"]
	metadata36 = jldopen("cluster/Kitaev36/metadata.jld2")["metadata"]
...
begin
	circuits24 = metadata24["circuits"]
	circuits28 = metadata28["circuits"]
	circuits32 = metadata32["circuits"]
	circuits36 = metadata36["circuits"]
...
begin
	pxs24 = [circuits24[i].params[1] for i in 1:100]
	pxs28 = [circuits28[i].params[1] for i in 1:100]
	pxs32 = [circuits32[i].params[1] for i in 1:100]
	pxs36 = [circuits36[i].params[1] for i in 1:100]
...
begin
	I3_24 = [mean(readdlm("cluster/Kitaev24/idx$(i).txt")) for i in 1:100]
	I3_28 = [mean(readdlm("cluster/Kitaev28/idx$(i).txt")) for i in 1:100]
	I3_32 = [mean(readdlm("cluster/Kitaev32/idx$(i).txt")) for i in 1:100]
	I3_36 = [mean(readdlm("cluster/Kitaev36/idx$(i).txt")) for i in 1:100]
	...
	ΔI3_36 = [std(readdlm("cluster/Kitaev36/idx$(i).txt")) ./sqrt(length(readdlm("cluster/Kitaev36/idx$(i).txt"))) for i in 1:100]
...
begin
	...
	pxs_new = reduce(vcat, [rangeA, rangeB, rangeC])
	println(length(pxs_new))
end
...
xs, ys, _, Ls = ScalingCollapse.scaled_data(sp)

# ╔═╡ 4036a087-0fba-43a5-9a15-304be10550ce
# ╠═╡ disabled = true
#=╠═╡
begin
	fig = Figure()
...
# ╔═╡ 36d5e36c-affd-4f51-9e01-9ce08efadd0e
# ╠═╡ disabled = true
#=╠═╡
begin
	fig = Figure()
	ax = Axis(fig[1,1])
	#scatter!(ax, pxs24, I3_24)
	#scatter!(ax, pxs28, I3_28)
	#scatter!(ax, pxs32, I3_32)
	#scatter!(ax, pxs36, I3_36)
	#scatter!(ax, pxs40, I3_40)
	scatter!(ax, pxs44, I3_44)
	scatter!(ax, pxs48, I3_48)
	scatter!(ax, pxs_new, zeros(length(pxs_new)), markersize=3)
	fig
end
  ╠═╡ =#

# ╔═╡ 798ac9ea-5d78-459e-a8db-822063bba768
begin
	A = ms.Point(-0.5, -sqrt(3)/4, 0.0)
	B = ms.Point(0.5, -sqrt(3)/4, 0.0)
	C = ms.Point(0.0, sqrt(3)/4, 0.0)
	D = ms.Point(0.0, 0, sqrt(3)/2)
	points = [A,B,C,D] #.- Meshes.Point(0.0, -sqrt(3)/4, 0.0)
	tris = ms.connect.([(1,2,3), (1,2,4), (2,3,4), (3,1,4)], ms.Triangle)
	mesh = ms.SimpleMesh(points, tris)
	cyl = ms.CylinderSurface(ms.Point(0.0, 0.0, 0.0), D, 0.3)
	cyl_mesh = ms.discretize(cyl, ms.RegularDiscretization(10))
	fig = cm.Figure()
	ax = cm.Axis3(fig[1,1])
	#cm.mesh!(ax, cyl_mesh.vertices, cyl_mesh.topology.connec)
	fig
end

# ╔═╡ 2c25e0d9-82ca-476b-a258-6f711403b261
cyl_mesh.topology.elems
...
ScalingCollapse = "bcf2b353-04f3-4f78-ac0b-80007e341812"
...
manifest_format = "2.0"
project_hash = "0a6c2ee8a119a0591cfa7ac3c7dd1e325792e8dd"
...
git-tree-sha1 = "0c3c650fbda06aa5b92afadd52ae9ad70fb2eff8"
...
[[deps.ScalingCollapse]]
deps = ["Dierckx", "KissSmoothing", "Measurements", "Optim", "Statistics"]
git-tree-sha1 = "c722d0502090e1fce12b99362edf212bc08adeb0"
uuid = "bcf2b353-04f3-4f78-ac0b-80007e341812"
version = "0.2.1"

    [deps.ScalingCollapse.extensions]
    InteractiveScalingCollapse = "GLMakie"

    [deps.ScalingCollapse.weakdeps]
