...
# ╔═╡ f3504fc0-1d2a-11ef-2ccb-bb0eb3d28ace
begin
	using GLMakie
...
# ╔═╡ 77bc81dd-4602-4c5f-a5de-2516db70e18f
metadata = jldopen("metadata.jld2")["metadata"]

# ╔═╡ 77f19cb4-33bb-4aab-87d2-c85b039538dd
px = [metadata["circuits"][i].params[1] for i in eachindex(metadata["circuits"])]

# ╔═╡ ac72787e-1200-475c-9e82-591ec720cd8d
EEs = [vec(mean(readdlm("idx$(i)_EE.txt"), dims=1)) for i in eachindex(metadata["circuits"])]

# ╔═╡ 16cc03d0-5e55-455b-9f8e-2f2348c034e2
TMIs = [(mean(readdlm("idx$(i)_I3.txt"))) for i in eachindex(metadata["circuits"])]

# ╔═╡ f7cd953c-0e4a-46f4-ac85-f174006207c1
for i in eachindex(EEs)
	EEs[i][2:end-1] .= EEs[i][2:end-1] .- minimum(EEs[i][2:end-1])
end

# ╔═╡ ef042a8f-1478-4a2a-b07b-a334a0fb096b


# ╔═╡ 39bbc88b-5434-4543-97eb-329d2ca6a95f
length(EEs[1])

# ╔═╡ d29057bc-d42f-431b-a86f-e0395f65e480
with_theme(merge(theme_minimal(), theme_latexfonts())) do 
	fig = Figure(size=(600,400))
	ax = Axis(fig[1,1], xlabel=L"\frac{1}{3} \log (\sin (\pi l/L))", ylabel=L"\Delta S_{vN}", title = L"Kitaev n.n. neighbour tetrahedron edge @ $L=36$")
	for i in eachindex(EEs)
		if 1-px[i] ≈ 1/3
			mark = :utriangle
			c = :black
			...
			mark = :circle
			c = get(ColorSchemes.tol_vibrant, (px[i]-0.5)*2)
		end
		scatterlines!(ax, 1/3 .*log.(sin.((1:1:35).*(pi/36))), EEs[i][2:end-1], color=c, marker=mark)
		
	end
	Colorbar(fig[1,2], limits=(1-maximum(px), 1-minimum(px)), label=L"p_J", labelsize=16, colormap=Reverse(ColorSchemes.tol_vibrant))
	save("EE_scaling2.png", fig, px_per_unit=4)
	fig
end

# ╔═╡ 9d113deb-fb0e-4c97-b81f-b8a2ca860b64
with_theme(merge(theme_minimal(), theme_latexfonts())) do 
	fig = Figure(size=(600,400))
	ax = Axis(fig[1,1], xlabel=L"p_J", ylabel=L"I3", title = L"Kitaev n.n. neighbour tetrahedron edge @ $L=36$")
	
	scatterlines!(ax, 1 .-px, TMIs)
	vlines!(ax, [1/3])
...
manifest_format = "2.0"
project_hash = "805ae9637785f86d2b9d04c5a519871fa81ba469"
