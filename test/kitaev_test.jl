...
circuits = [QMOC.KitaevCircuit(L, params) for params in parameter_set]
n_states = Threads.nthreads()
n_samples = 100
...
p1 = [parameter_set[i][1] for i in eachindex(parameter_set)]
p2 = [parameter_set[i][2] for i in eachindex(parameter_set)]
p3 = [parameter_set[i][3] for i in eachindex(parameter_set)]
I3 = I3

data = [p1 p2 p3 I3]
writedlm("kitaev_wedge_$(L).txt", data)
